...
(function(){
    ...
    function getCurrentCity(){let txt=document.body.innerText; let m=txt.match(/Black Market - ([A-Za-z ]+)/i); if(m) return m[1].trim(); m=txt.match(/You have traveled to ([A-Za-z ]+)!/i); if(m) return m[1].trim(); let cur=[...document.querySelectorAll('*')].find(e=>e.innerText&&e.innerText.trim()==='CURRENT LOCATION'); if(cur){let p=cur.closest('div').innerText.toUpperCase(); for(let c in CITY_IDS) if(p.includes(c.toUpperCase())) return c;} let sm=txt.match(/Location\s*\n?\s*(Cairo|Tokyo|London|Moscow|Rome|Capetown|Sydney|Ottawa|Rio de Janeiro)/i); if(sm) return sm[1].trim(); return null;}
    ...
    function build(){
        ...
        updateList();
        document.getElementById('pd-from-sel').addEventListener('change',e=>{let city=CITIES[e.target.value]; localStorage.setItem('pd_from',city); localStorage.setItem('pd_manual_from','1'); if(cityConfig[city]){localStorage.setItem('pd_item',cityConfig[city].item); localStorage.setItem('pd_to',cityConfig[city].to); document.getElementById('pd-item-sel').value=ITEMS.indexOf(cityConfig[city].item); document.getElementById('pd-to-sel').value=CITY_IDS[cityConfig[city].to]||1;} });
        document.getElementById('pd-item-sel').addEventListener('change',e=>{let it=ITEMS[e.target.value]; let from=CITIES[document.getElementById('pd-from-sel').value]; let to=CITIES[document.getElementById('pd-to-sel').value]; saveCfg(from,it,to);});
        document.getElementById('pd-to-sel').addEventListener('change',e=>{let to=CITIES[e.target.value]; let from=CITIES[document.getElementById('pd-from-sel').value]; let it=ITEMS[document.getElementById('pd-item-sel').value]; saveCfg(from,it,to);});
        function doStart(){
