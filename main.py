...
def human_click(page, locator, timeout=8000):
    """
    كليك حقيقي بحركة ماوس بشرية
    - scroll للأزرار
    - حركة ماوس في منحنى
    - كليك حقيقي (isTrusted = True)
    """
    try:
            ...
            try:
                ...
                for attempt in range(15):
                    try:
                        ...
                        if check_if_jailed(page):
                            print(f"🚔 [الأسهم] سجن أثناء الشراء!")
                            for _ in range(JAIL_WAIT // 60):
                                time.sleep(60)
                                if should_stop():
                                    stop_bot()
