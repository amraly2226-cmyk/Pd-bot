...
def run_theft_bot():
    ...
    STUCK_WAIT = 5 * 60  # 5 دقايق لو حصلت مشكلة
        ...
        try:
            with sync_playwright() as p:
                ...
                try:
                    ...
                    browser.close()
                    print(f"⏰ [Theft] هستنى {STUCK_WAIT // 60} دقايق وأحاول تاني...")
                ...
                while True:
                    try:
                        ...
                        # ✅ لو حصل أي إيرور (زي السجن أو تعليق الصفحة)
                        print(f"⚠️ [Theft] حصلت مشكلة: {e}")
                        print(f"⏰ [Theft] هستنى {STUCK_WAIT // 60} دقايق وأحاول تاني...")
                        time.sleep(STUCK_WAIT)
                        # نخرج من الـ loop الداخلي عشان نعيد إنشاء المتصفح
                        break
