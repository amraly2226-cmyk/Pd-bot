  ...
  while (true) {
    try {
      let state = await page.evaluate((items) => {
        ...
        // 🔥 قراءة الكمية (Hold) بشكل مباشر وآمن
        let hold = 0;
      ...
      if (state.cd) {
        ...
        for (let i = 0; i < 3; i++) {
            ...
            if (currentHold === 0) {
                console.log("📦 فحصت السوق: مش شاري حاجة حالياً (هيتم الشراء فور انتهاء الكولداون)");
            } else {
                console.log(`✅ فحصت السوق: كمية ${currentHold} (شاريها، مش هشتري تاني)`);
            }
      ...
      // ✅ كايرو: شراء فقط لو فاضي تماماً، وبعدها سافر فوراً
      if (state.loc === "Cairo") {
        if (state.hold > 0) {
           console.log(`📍 كايرو - معاك ${state.hold} قطعة، مش هشتري، رايح طوكيو`);
           await page.goto('https://www.project-dark.co.uk/travel', { waitUntil: 'networkidle2' });
           await sleep(2500);
           // فحص الكولداون هنا
           ...
           if (travelCd) {
               console.log(`⏳ لقيت كولداون في السفر: ${travelCd}`);
               continue;
           }
           // لو مفيش كولداون، كمّل السفر
        ...
        if (state.hold === 0) {
           console.log("📍 كايرو - فاضي، هشتري أنابوليك مرة واحدة وبعدها أسافر");
      ...
      // ✅ طوكيو: شراء فقط لو فاضي تماماً، وبعدها سافر فوراً
      else if (state.loc === "Tokyo") {
        if (state.hold > 0) {
           console.log(`📍 طوكيو - معاك ${state.hold} قطعة، مش هشتري، رايح كايرو`);
           await page.goto('https://www.project-dark.co.uk/travel', { waitUntil: 'networkidle2' });
           await sleep(2500);
           // فحص الكولداون هنا
           let travelCd = await page.evaluate(() => {
        ...
        if (state.hold === 0) {
           console.log("📍 طوكيو - فاضي، هشتري إلكترونيكس مرة واحدة وبعدها أسافر");
